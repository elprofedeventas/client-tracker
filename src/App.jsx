...
function ViewOrder({ order, onBack, onChangeEstado, showToast, backLabel = 'Volver a órdenes' }) {
  ...
  return (
    <div style={{ animation: 'fadeUp 0.4s ease' }}>
      ...
      {/* Botones de exportar — solo cuando Vendido */}
      {estado === 'Vendido' && (() => {
...
Total:             ${fmtMoney(order.total)}`
        ...
        const waPhone = order.clienteTelefono
          ? `593${order.clienteTelefono.replace(/\D/g,'').replace(/^0/,'')}`
          : ''
        const waLink    = `https://wa.me/${waPhone}?text=${encodeURIComponent(mensaje)}`
        const emailLink = `mailto:${order.clienteEmail || ''}?subject=${encodeURIComponent(`Orden ${order.numOrden} — ${order.clienteNombre}`)}&body=${encodeURIComponent(mensaje)}`
      ...
      {/* Enviar datos para facturar */}
      {facturacion && (() => {
        const lineas = (order.items || []).map(it =>
          `• ${it.nombre}  x${it.cantidad}  $${parseFloat(it.total||0).toFixed(2)}`
        ).join('\n')
        const mensaje =
          `*Datos para facturar*\n\n` +
          `*Cliente:* ${order.clienteNombre}\n` +
          ...
          `*Orden:* ${order.numOrden}\n` +
          `${lineas}\n\n` +
          `*Subtotal:*  $${parseFloat(order.totalSinIva||0).toFixed(2)}\n` +
          `*IVA:*       $${parseFloat(order.totalIva||0).toFixed(2)}\n` +
          `*Total:*     $${parseFloat(order.total||0).toFixed(2)}`

        const waUrl = `https://wa.me/${facturacion.celular}?text=${encodeURIComponent(mensaje)}`
        const mailUrl = `mailto:${facturacion.email}?subject=${encodeURIComponent(`Facturar orden ${order.numOrden} – ${order.clienteNombre}`)}&body=${encodeURIComponent(mensaje.replace(/\*/g, ''))}`
        ...
        return (
          <div style={{ background: 'var(--white)', border: '1.5px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '18px 20px', boxShadow: 'var(--shadow)' }}>
            <div style={{ fontSize: '11px', fontWeight: '700', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '12px' }}>
              Enviar datos de la orden para facturar
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <a href={waUrl} target="_blank" rel="noopener noreferrer"
                style={{ flex: 1, padding: '11px', background: '#16a34a', color: 'white', borderRadius: 'var(--radius)', fontSize: '13px', fontWeight: '700', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px', transition: 'background 0.15s' }}
              ...
              <a href={mailUrl}
                style={{ flex: 1, padding: '11px', background: '#2563eb', color: 'white', borderRadius: 'var(--radius)', fontSize: '13px', fontWeight: '700', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px', transition: 'background 0.15s' }}
